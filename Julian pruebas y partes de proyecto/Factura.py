...
def calcular():
    ...
    if compro == True:
        ...
        resultadoT.set("Subtotal: "+str(float(resultado1.get())+float(resultado2.get())+float(resultado3.get())+float(resultado3.get())+float(resultado4.get())+float(resultado5.get())+float(resultado6.get())))
...
tk.Label(text="Cliente: ", bg= "while").place(x=10, y=40)
...
tk.Button(text="limpiar",width=10,heigth=1, bg= "Orange", command=limpiar).place(x=570, y=40)
...
tk.Label(text="Jefes: el gordo",bg="white").place(x=10,y=460)
...
tk.Entry(textvariable= producto6,bd=1,width=12,justify="left").place(x= 90, y=260)
...
tk.Label(texvariable= resultadoTIVA, justify="left").place(x=480,y=10)
